...
export default function AdminPage() {
  ...
  useEffect(() => {
    ...
    // Redirigir automáticamente al admin del club del usuario
    if (user.clubId) {
      const redirectToClubAdmin = async () => {
        try {
          ...
          // Si falla, mantener en /admin (compatibilidad)
          setAuthChecked(true);
        }
      };
      redirectToClubAdmin();
    } else {
      // Si no tiene clubId, mantener en /admin (compatibilidad temporal)
      setAuthChecked(true);
    }
  }, [router]);
  ...
  const loadClub = async () => {
    try {
      ...
      // Si no hay clubId del usuario, obtener el primer club disponible
      if (!clubId) {
